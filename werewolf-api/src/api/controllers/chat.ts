...
async function requestToken(apiKey: string) {
  ...
  const result = await (async () => {
    ...
    .catch((err) => {
      if (accessTokenRequestQueueMap[apiKey]) {
        accessTokenRequestQueueMap[apiKey].forEach((resolve) => resolve(err));
        delete accessTokenRequestQueueMap[apiKey];
      }
      return err;
    });
    ...
    const parser = createParser((event) => {
      try {
        ...
        if (result.status != "finish") {
          ...
          if (!content) return;
          const { type, text, image, code, content: innerCcontent } = content;
          let innerStr = "";
          if (type == "text") {
            if (toolCall) {
              innerStr += "\n";
    ...
    const parser = createParser((event) => {
      try {
        ...
        if (result.status != "finish") {
          const { status, content, meta_data } = result.message;
          if (!content) return;
          const { type, text, image } = content;
          if (type == "image" && _.isArray(image) && status == "finish") {
            image.forEach((value) => {
              if (
